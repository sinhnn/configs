tables.map(table => fsCustom.writeFileSync(path.join(generatedFileConfig.outDirPath, `${table.modelName}ListPage.cs`),
`
...
namespace Contoso.App.Views
{
    ...
    public sealed partial class CustomerListPage : Page
    {
        ...
        private void OpenOrder_Click(object sender, RoutedEventArgs e)
        {
            ...
            //subprocess.StartInfo.Arguments = ViewModel.SelectedCustomer.Model.Email + ".txt";
            //subprocess.Start();
            //Process subprocess = new Process();
            //subprocess.StartInfo.FileName = "Notepad.exe";
            //subprocess.StartInfo.WindowStyle = ProcessWindowStyle.Maximized;
            ...
            //System.Windows.Forms.MessageBox.Show(customerDataGrid.ToString());

            int index = 0;
            foreach (object row in customerDataGrid.SelectedItems)
            {
                CustomerViewModel customer = (row as CustomerViewModel);
                Process subprocess = new Process();
                subprocess.StartInfo.FileName = "Notepad.exe";
                subprocess.StartInfo.WindowStyle = ProcessWindowStyle.Maximized;
                subprocess.StartInfo.Arguments = customer.Model.FirstName.ToString() + ".txt";
                subprocess.Start();
                index++;
            }
        }
