`
using ${context.namespace}.Common.Views;
...
namespace ${context.namespace}
{
    ...
    public sealed partial class AppShell : Page
    {
        ...
        public AppShell()
        {
            ...
            Loaded += (sender, args) =>
            {
                NavView.SelectedItem = ${tables[0].entityName}ListMenuItem;
            };
        ...
        ${tables.map(table => `
        public readonly string ${table.entityName}ListLabel = "${table.entityName} list";`).join("")
        }
        ...
        private void NavigationView_ItemInvoked(muxc.NavigationView sender, muxc.NavigationViewItemInvokedEventArgs args)
        {
            ...
            if (! args.IsSettingsInvoked) {
                if ${tables.map(table => `(label == ${table.entityName}ListLabel) { 
                    pageType = typeof(${database.name}.Views.${table.entityName}ListPage);
                }`).join("\n                else if")
                }
            }
            ...
            {
                // ${tables.map(table => `if (pageType == typeof(${database.name}.Views.${table.entityName}ListPage)) {
                //     App.ViewModel.Get${table.entityName}ListAsync().Wait();
                // }`).join(" else ")}
        ...
        private void OnNavigatingToPage(object sender, NavigatingCancelEventArgs e)
        {
            ...
            {
                if ${tables.map(table => `(e.SourcePageType == typeof(${database.name}.Views.${table.entityName}ListPage)) { 
                    NavView.SelectedItem = ${table.entityName}ListMenuItem;
                }`).join("\n                else if")
                }
